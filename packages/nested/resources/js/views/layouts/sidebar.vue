    ...
    <div class="flex flex-col w-64">
      ...
      <div class="flex flex-col h-0 flex-1">
        <router-link to="/" custom v-slot="{ href, navigate }">
          <a :href="href" @click="navigate" class="flex items-center h-16 flex-shrink-0 px-4 bg-gray-900">
            <svg class="h-8" style="color: #6875f5;" fill="none" viewBox="0 0 34 32" xmlns="http://www.w3.org/2000/svg">
            ...
            <span class="text-white ml-2.5 font-semibold" style="font-size: 22px;">Board</span>
          </a>
        </router-link>
            ...
            <template v-for="item in items" :key="item.label">
              ...
              >
                <a :href="href" @click="navigate" :class="[isActive ? activeClass : defaultClass]">
                  <!-- <v-svg :name="item.icon" :classes="isActive ? 'mr-3 h-6 w-6 text-gray-300' : 'mr-3 h-6 w-6 text-gray-400 group-hover:text-gray-300'"/> -->

                  <!-- <v-svg name="plus"/> -->
                  <component :is="'svg-'+item.icon" :class="isActive ? 'mr-3 h-6 w-6 text-gray-300' : 'mr-3 h-6 w-6 text-gray-400 group-hover:text-gray-300'"/>

                  <!-- <svg xmlns="http://www.w3.org/2000/svg" :class="isActive ? 'mr-3 h-6 w-6 text-gray-300' : 'mr-3 h-6 w-6 text-gray-400 group-hover:text-gray-300'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                  </svg> -->

                  {{ $t(item.label, 2) }}
                  <span class="ml-auto cursor-pointer w-5 h-5 rounded bg-gray-700 text-white hover:bg-gray-800 group-hover:bg-gray-500 hover:opacity-250 hover:text-gray-300 flex items-center justify-center" @click.prevent="add(item.add)" v-if="item.add">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" width="16" stroke="currentColor">
...
<script>
  ...
  import SvgCog from '@/components/svg-cog.vue'
